...
public final class OperationDeliveryService {
    ...
    public OperationDeliveryService(Path repository, String remote, String branch, String repositorySlug, String migrationWorkflow, String publishWorkflow, OperationManager operations) {
        this.git=new GitService(repository); this.workflows=new WorkflowService(repository,repositorySlug); this.remote=remote;this.branch=branch;this.migrationWorkflow=migrationWorkflow;this.publishWorkflow=publishWorkflow;this.operations=operations;
    }
    public DeliveryResult deliver(OperationManager.PreparedOperation operation, Path repository) throws IOException {
        ...
        if (!"OK".equals(m.steps.get("commit"))) { git.requireBranch(branch); git.requireDiffCheck(); git.stageExact(files); git.commit(commitMessage(m)); m.steps.put("commit","OK"); m.status="COMMITTED"; operations.save(operation); }
